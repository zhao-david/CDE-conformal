...
def difference_validity_and_efficiency(true_cde,
    ...
    estimates at an individual level (assumes some smoothness of both cs values).
    ...
    Additional Notes:
    -----------------
    In this function, the efficiency analysis doesn't work when the true
    conformal scores are non-continuous (e.g. Uniform distributions with HDP as
    conformal scores). For uniform distributions in general (cs as CDE or HDP)
    try `lc.difference_efficiency_uniform`.
    """
    ...
    for t_idx in threshold_iter:
        ...
        # which part of the grid has a predicted CS score above the prediction based threshold?
        ...
        # which of the observations should be inside the prediction region...
...
def difference_efficiency_uniform(true_cde,
                                  predict_grid,
                                  true_grid,
                                  thresholds_predict,
                                  thresholds_true,
                                  ...
                                  verbose = True):
    """
    Calculates difference in efficiency between predicted and true
    estimates at an individual level if the true_cde is a uniform distribution.
    ...
    if np.any(thresholds_true <= 0):
            warning("a uniform threshold is below 0..."+\
                    "we are assuming that cs values for with"+\
                    "non-zero prob have cs above 0")
    ...
    for t_idx in threshold_iter:
        ...
        unif_value = true_cde.max(axis = 1)

        mass_of_intersect = (true_cde*predict_indicator_ge_threshold).sum(axis = 1)*z_delta

        error_in_mass_of_intersect = np.abs(mass_of_intersect - expected_p)

        # now get error in lebegue distance
        efficiency_error[:, t_idx] = error_in_mass_of_intersect/unif_value

    if true_grouping is None:
        return efficiency_error
    else:
        _, efficiency_average = lc.average_within_groups(true_grouping,
                                                      efficiency_error)
