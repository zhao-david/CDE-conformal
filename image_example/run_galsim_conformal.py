...
def sample_lambda_range(sample_size, lambda_min=0.75, lambda_max=0.85):
    alpha_sample = np.random.uniform(-math.pi, math.pi, size=sample_size)
    lambda_sample = np.random.uniform(lambda_min, lambda_max, size=sample_size)
...
def main(sample_size, main_peak, save_out=True, mixing_param=0.5, downsampling=20,
         lambda_min=0.75, lambda_max=0.85, alpha_value=0, lambda_value=0.4):
    """
    ...
    # Sample params directly from "prior"
    alpha_sample, lambda_sample = sample_lambda_range(
        sample_size=sample_size, lambda_min=lambda_min, lambda_max=lambda_max)

    # 5/20 even simpler: just define what alphas and lambdas should be
    alpha_sample = np.random.uniform(alpha_value - 0.3, alpha_value + 0.3, size=sample_size) #np.repeat(alpha_val, sample_size)
    lambda_sample = np.random.uniform(lambda_value - 0.1, lambda_value + 0.1, size=sample_size) #np.repeat(lambda_val, sample_size)
    ...
    if save_out:
        outfile_name = 'data/galsim_simulated_%sgals_lambda%s_%s_%s.pkl' % (
            final_sample.shape[0], lambda_min, lambda_max,
            datetime.strftime(datetime.today(), '%Y-%m-%d-%H-%M')
        )
        
        # 5/20 file name
        outfile_name = 'data/galsim_simulated_%sgals_lambda%s_theta%s_%s.pkl' % (
            final_sample.shape[0], lambda_value, alpha_value,
            datetime.strftime(datetime.today(), '%Y-%m-%d-%H-%M')
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_min', action="store", type=float, default=0.75,
                        help='Lower limit for lambdas to sample.')
    parser.add_argument('--lambda_max', action="store", type=float, default=0.85,
                        help='Upper limit for lambdas to sample.')
    parser.add_argument('--alpha_val', action="store", type=float, default=None,
                        help='Simulated alpha value.')
    parser.add_argument('--lambda_val', action="store", type=float, default=None,
                        help='Simulated lambda value.')
    ...
    main(
        ...
        lambda_min=argument_parsed.lambda_min,
        lambda_max=argument_parsed.lambda_max,
        lambda_value=argument_parsed.lambda_val,
        alpha_value=argument_parsed.alpha_val
    )